      ...
      await User.findOne({ email }).then(async (user) => {
        if (!user) {
          return done(
            ...
            req.flash(
              "error-message",
              "This email doesn't exist. Please use a correct email address."
            )
          );
        }
        bcrypt.compare(password, user.password, (err, passwordMatch) => {
          ...
          if (!passwordMatch)
            return done(
              ...
              req.flash(
                "error-message",
                "Wrong pasword. Please check your password and try again."
              )
            );
...
app.get("/campaign/single-campaign/:campaignId", async (req, res) => {
  ...
  if (!singleCampaign) {
    req.flash(
      "error-message",
      "<b>Error:</b> Invalid campaign link! Please check the link and try again."
    );
    return res.redirect("/user/login");
  } else {
